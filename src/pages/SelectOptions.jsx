...
const SelectOptions = () => {
  ...
  const [step, setStep] = useState(3); // Step 1: Cuisine, Step 2: Spice Level, Step 3: Chutney
  ...
  const handleContinue = () => {
    if (step === 3 && selectedCuisine) {
      setStep(4);
    } else if (step === 4 && selectedSpiceLevel) {
      setStep(5);
    } else if (step === 5 && selectedChutneys.length <= chutneyLimit) {
      navigate("/pizzas", {
        state: {
          limits,
          reservationId,
